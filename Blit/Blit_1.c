...
__saveds void myBlitBoardHook(register __a0 struct Hook *hook, register __a2 struct RastPort *rp, register __a1 struct blitMsg *bm)
{
	...
	/* Setup board */
	for (y = dr.MinY; y < dr.MaxY; y += 16)
	{
		for (x = dr.MinX; x < dr.MaxX; x += 16)
		{
			WORD src = table[y >> 4][x >> 4];
			bltTileBitMap(bi->src, src << 4, 1 << 4, rp->BitMap, x, y, 16, 16);
		}
	}
}
