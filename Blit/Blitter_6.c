...
{
    struct Rectangle *rect = &blitMsg->bounds;
    ...
    drawTile(bi->srcBitMap, bi->srcX + blitRect.MinX - bi->destX, bi->srcY + blitRect.MinY - bi->destY, rPort->BitMap, rect->MinX + blitRect.MinX - blitMsg->offsetX, rect->MinY + blitRect.MinY - blitMsg->offsetY, blitRect.MaxX - blitRect.MinX + 1, blitRect.MaxY - blitRect.MinY + 1);
}
...
void drawTileRastPort(struct BitMap *srcBitMap, WORD srcX, WORD srcY, struct RastPort *destRPort, WORD destX, WORD destY, WORD bltWidth, WORD bltHeight)
{
    ...
    DoHookClipRects(&hook, destRPort, clip);
}
