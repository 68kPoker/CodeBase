...
LEV *loadLevel(STRPTR name)
{
    ...
    if (lev = AllocMem(sizeof(*lev), MEMF_PUBLIC))
    {
        if (iff = openIFF(name, FALSE))
        {
            if (PropChunk(iff, ID_MAGA, ID_PLAN) == 0)
            {
                if (StopOnExit(iff, ID_MAGA, ID_FORM) == 0)
                {
                    err = ParseIFF(iff, IFFPARSE_SCAN);
                    if (err == 0 || err == IFFERR_EOC || err == IFFERR_EOF)
                    {
                        struct StoredProperty *sp;
                        if (sp = FindProp(iff, ID_MAGA, ID_PLAN))
                        {
                            *lev = *(struct Level *)sp->sp_Data;
                            closeIFF(iff);
                            return(lev);
                        }
                    }
...
BOOL saveLevel(LEV *lev, STRPTR name)
{
    ...
    if (iff = openIFF(name, TRUE))
    {
        if (PushChunk(iff, ID_MAGA, ID_FORM, IFFSIZE_UNKNOWN) == 0)
        {
            if (PushChunk(iff, ID_MAGA, ID_PLAN, sizeof(*lev)) == 0)
            {
                if (WriteChunkBytes(iff, lev, sizeof(*lev)) == sizeof(*lev))
                {
                    if (PopChunk(iff) == 0)
                    {
                        if (PopChunk(iff) == 0)
